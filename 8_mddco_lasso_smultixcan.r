...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/smultixcan_2_lasso/mddco')
...
#Save final result
	fwrite(as.data.frame(target.gene.order),file=paste0('mddco.finalresult.txt'),sep=' ')
	fwrite(as.data.frame(assessment_sum),'/exeh_4/jinghong_qiu/SO_Lab/Contribution/smulti-mddco.lasso.csv',sep=',')
	sum_data = data.frame(qval,fdr.the,fdr.emp,bio.fdr.the,bio.fdr.emp,bio.fdr.the.lasso,bio.fdr.emp.lasso)
	fwrite(as.data.frame(sum_data),'/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/smulti-mddco.lasso.csv',sep=',')	
	save(fdr.theore,fdr.empiri,fdr.target.theore,fdr.target.empiri,lasso.fdr.target.theore,lasso.fdr.target.empiri,file = "/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/smulti-mddco.lasso.RData")
#exit the software
	q('no') 