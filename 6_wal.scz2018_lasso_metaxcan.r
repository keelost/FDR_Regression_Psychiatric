...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/metaxcan_2_lasso/wal.scz2018')
...
#read all predixcan results in cerebellum region
	...
	target_rank = which(str_detect(filenames,'wal.scz2018'))
...
#extract target zscore and construct variables z.score matrix 
	...
	colnames(z.combine)=c('ad','adhd2019','alco2018','anx','asd2019','bd2018','cannabis','chrono','dx.dep','dx.life.threat.ill','ed2019','ever.csh','ever.sh','ever.st','life.threat.accident','mdd2019','mddco','neuroti2018','no.dep','ocd2018','pd','physical.crime','psycho.experi','ptsd2019','reac','sleep.dura','smoking','st','st.dep','tense','wit.death')
...
#Save final result
	...
	fwrite(as.data.frame(sum_data),paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/meta-wal.scz2018.lasso-',brain_region,'.test_SetSeed.csv'),sep=',')
	save(fdr.theore,fdr.empiri,fdr.target.theore,fdr.target.empiri,lasso.fdr.target.theore,lasso.fdr.target.empiri, file =paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/meta-wal.scz2018.lasso-',brain_region,'.test_SetSeed.RData'))
	fwrite(as.data.frame(target.gene.order),file=paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/smultixcan_1_all/wal.scz2018/Result/wal.scz2018.lasso.finalresult.',brain_region,'.test_SetSeed.txt'),sep=' ')
	assessment.final$region = brain_region
	assessment.final.sum = rbind(assessment.final.sum,assessment.final)
	fwrite(as.data.frame(assessment.final),file=paste0('/exeh_4/jinghong_qiu/SO_Lab/Contribution/meta-wal.scz2018.lasso-',brain_region,'.test_SetSeed.csv'),sep=',')
}
	fwrite(as.data.frame(final_sum),file='/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/meta-wal.scz2018.lasso.all.test_SetSeed100.csv',sep=',')
	fwrite(as.data.frame(assessment.final.sum),file='/exeh_4/jinghong_qiu/SO_Lab/Contribution/meta-wal.scz2018.lasso-all.test_SetSeed100.csv',sep=',')
#exit the software
	q('no') 