...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/metaxcan_1_all/mdd2013')
...
#read all predixcan results in cerebellum region
	...
	target_rank = which(str_detect(filenames,'mdd2013'))
...
#extract target zscore and construct variables z.score matrix 
	...
	colnames(z.combine)=c('adhd2019','alco2018','antisocial','anx','asd2019','bd2018','cannabis','chrono','dx.dep','ed2019','ever.csh','ever.sh','ever.st','insom2019','inte','life.threat.accident','neo.c','neuroti2018','no.dep','physical.crime','psycho.experi','ptsd2019','scz.eas2019','st','st.dep','wal.scz2018')
...
#Save final result
	...
	fwrite(as.data.frame(sum_data),paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/meta_mdd2013_',brain_region,'.test_SetSeed.csv'),sep=',')
	save(fdr.theore,fdr.empiri,fdr.target.theore,fdr.target.empiri,lasso.fdr.target.theore,lasso.fdr.target.empiri, file =paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/meta_mdd2013_',brain_region,'.test_SetSeed.RData'))
	fwrite(as.data.frame(target.gene.order),file=paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/smultixcan_1_all/mdd2013/Result/mdd2013.finalresult.',brain_region,'.test_SetSeed.txt'),sep=' ')
	assessment.final$region = brain_region
	assessment.final.sum = rbind(assessment.final.sum,assessment.final)
	fwrite(as.data.frame(assessment.final),file=paste0('/exeh_4/jinghong_qiu/SO_Lab/Contribution/meta-mdd2013-',brain_region,'.test_SetSeed.csv'),sep=',')
}
	fwrite(as.data.frame(final_sum),file='/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/meta_mdd2013.all.test_SetSeed300.csv',sep=',')
	fwrite(as.data.frame(assessment.final.sum),file='/exeh_4/jinghong_qiu/SO_Lab/Contribution/meta-mdd2013-all.test_SetSeed300.csv',sep=',')
#exit the software
	q('no') 