#!/bin/bash
# ============================================================
# Supplemental analysis: generate sumstats and calculate h2 and rg for omitted targets.
# Supports two N sources:
#   1) auto       - read from the source file's n column, matching the original munge step.
#   2) <integer>  - use a fixed sample size when the file has no n column.
...
# Format: "<source file path>|<N source>"
#   N source = "auto"     - read the n column from the first row.
#   N source = <integer>  - specify the sample size when no n column exists.
# Previously munged files are skipped automatically, so duplicate entries are harmless.
MISSING_RAW=(
...
for entry in "${MISSING_RAW[@]}"; do
    ...
    if [ -f "$sumstats" ]; then
        ...
        if [ ! -f "$raw" ]; then
            echo "  [ERROR] source file does not exist: $raw; skipping"
            continue
        fi
        ...
        if [ "$nspec" = "auto" ]; then
            # ---- Match the original munge logic: read one N from the first row ----
            n_col=$(head -1 "$raw" | tr ' \t' '\n\n' | grep -nx "n" | head -1 | cut -d: -f1)
            if [ -z "$n_col" ]; then
                echo "  [ERROR] $base has no n column; use an integer N spec or skip this file"
                continue
            fi
            ...
            # ---- Use a fixed N when the file has no n column ----
            # Validate that N is a positive integer.
            if ! [[ "$nspec" =~ ^[0-9]+$ ]]; then
                echo "  [ERROR] invalid N spec for $base: '$nspec' (expected 'auto' or a positive integer); skipping"
                continue
            fi
            echo "        N = $nspec (fixed value; file has no n column)"
