...
option_list <- list(
  ...
  make_option(c("--out_dir"), type = "character",
              default = file.path(Sys.getenv("FDRREG_RESULTS_DIR", ""), "01.extra.analysis", "10.ablation", "smultixcan_v7"),
...
detect_targets <- function(base_dir) {
  ...
  dirs[vapply(dirs, function(d)
    dir.exists(file.path(base_dir, d, "08.smultixcan_output_v7")), logical(1))]
}
...
for (target in targets) {
  ...
  input_dir <- file.path(opt$base_dir, target, "08.smultixcan_output_v7")
