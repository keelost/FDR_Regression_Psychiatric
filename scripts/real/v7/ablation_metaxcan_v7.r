...
option_list <- list(
  ...
  make_option(c("--out_dir"), type = "character",
              default = file.path(Sys.getenv("FDRREG_RESULTS_DIR", ""), "01.extra.analysis", "10.ablation", "metaxcan_v7"),
...
detect_targets <- function(base_dir) {
  ...
  dirs[vapply(dirs, function(d)
    dir.exists(file.path(base_dir, d, "06.metaxcan_v7")), logical(1))]
}
...
for (target in targets) {
  cat(sprintf("\n========== MetaXcan target: %s ==========\n", target))
  input_dir <- file.path(opt$base_dir, target, "06.metaxcan_v7")
