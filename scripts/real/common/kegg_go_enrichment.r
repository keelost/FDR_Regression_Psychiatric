...
if (!nzchar(input_root_dir)) stop("Set FDRREG_RESULTS_DIR before running KEGG/GO enrichment.")

# Sub-directory markers that distinguish the two analysis methods.
magma_subdir_marker      <- "05.magma_fdrreg"
smultixcan_subdir_marker <- "09.smultixcan_fdrreg"
...
output_dir <- file.path(input_root_dir, "01.extra.analysis", "04.kegg_go")
...
process_file <- function(file_path, method_label, gene_column) {
  ...
  significant_rows <- gene_stat_table[[SIG_FDR_COLUMN]] < SIG_FDR_THRESHOLD
