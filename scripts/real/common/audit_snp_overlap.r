...
results_root <- Sys.getenv("FDRREG_RESULTS_DIR", "")

# New input sources for requirement (2).
targets_dir <- Sys.getenv("FDRREG_TARGETS_DIR", "")   # target SNPs
library_dir <- Sys.getenv("FDRREG_LIBRARY_DIR", "")   # trait SNPs (.txt.gz)

# Where to write the audit summary (dedicated tmp/summary directory).
audit_dir <- file.path(results_root, "01.extra.analysis", "07.dist")
