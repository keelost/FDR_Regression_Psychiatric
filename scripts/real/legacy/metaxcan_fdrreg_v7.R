...
METAXCAN_SUBDIR <- "06.metaxcan_v7"
...
extract_trait_from_filename <- function(filename, brain_region) {
  ...
  trait_name <- sub("^gtex_v7_", "", trait_name)
...
run_target_analysis <- function(
  ...
  if (is.null(opt$output_dir)) {
    output_base <- file.path(opt$input_dir, TARGET, "07.metaxcan_fdrreg_v7")
  } else {
    output_base <- file.path(opt$output_dir, TARGET, "07.metaxcan_fdrreg_v7")
  }
