...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/smultixcan_1_all/bd2012')
...
#---------Lasso---------#
	...
	# simpleError in optim(start, fn = SoftLogitLoss, gr = SoftLogitGradient, method = "BFGS",     y = y, X = X, lambda = lambda, hessian = TRUE): initial value in 'vmmin' is not finite>
	# [1] "An error was encountered in fitting the regression.  Reverting to the PR no-covariates model."
...
#Save final result
	fwrite(as.data.frame(target.gene.order),file=paste0('bd2012.finalresult.txt'),sep=' ')
	#fwrite(as.data.frame(assessment_sum),'/exeh_4/jinghong_qiu/SO_Lab/Contribution/smulti-bd2012.csv',sep=',')
	sum_data = data.frame(qval,fdr.the,fdr.emp,bio.fdr.the,bio.fdr.emp)
	fwrite(as.data.frame(sum_data),'/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/smulti-bd2012.csv',sep=',')	
	save(fdr.theore,fdr.empiri,fdr.target.theore,fdr.target.empiri,file = "/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/smulti-bd2012.RData")
#exit the software
	q('no') 