	...
	#---- download MAGMA results ----#
	#set the path where you want to store the MAGMA results, example: "E:/Projects_documents/06.manuscript_polishment/20240916/test/"
	store_dir <- "your_path"
	download.file("https://raw.githubusercontent.com/keelost/FDR_Regression_Psychiatric/refs/heads/main/bd2018.target.covariables.zip",paste0(store_dir,"bd2018.target.covariables.zip"))
	
	#---- set dir and uncompress MAGMA results----#
	setwd(store_dir)
	unzip("bd2018.target.covariables.zip")

	#---- Pipeline ----#
	# (1) Load data
	# Target
	target.gene = fread(paste0(store_dir,"02/bd2018.genes.out"))
	...
	files = lapply(paste0(store_dir,'02/', listnames, '.genes.out'), fread, stringsAsFactor = F)
