...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_1_all/mddco/Result')
...
#Variables
	listnames = c('bd2018','cannabis','dx.dep','ed2019','edu','inte','neuroti2018','ocd2018','scz.eas2019','wal.scz2018')
	files=lapply(paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_1_all/mddco/Result/',listnames,'.genes.out'),fread,stringsAsFactor=F)
#data preproccess
...
#Save result	
	...
	fwrite(as.data.frame(assessment_sum),'/exeh_4/jinghong_qiu/SO_Lab/Contribution/magma-mddco.csv',sep=',')
	sum_data = data.frame(qval,fdr.the,fdr.emp,bio.fdr.the,bio.fdr.emp,bio.fdr.the.lasso,bio.fdr.emp.lasso)
	fwrite(as.data.frame(sum_data),'/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/magma_mddco.csv',sep=',')	
	save(fdr.target.theore,fdr.target.empiri,fdr.target.theore.bio,fdr.target.empiri.bio,fdr.target.theore.bio.lasso,fdr.target.empiri.bio.lasso,file = "/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/magma_mddco.RData")
#exit the software
	q('no') 