#****************************************#
#-----Drug Enrichment_pvalue.version-----#
#****************************************#
#R-4.0.2
#2020/11/13
...
#read magma results for FDRreg results (zscore-->pvalue-->magma)and rename Pvalue
    ...
    for (y in type.target){    
        ...
        if ('qval' %in% y){    
            colnames(trait_gene)[colnames(trait_gene)=='pvalue'|colnames(trait_gene)=='P'] <- "Pvalue"} else {
                trait_gene$p.est = trait_gene$bio.FDR.the*rank(trait_gene$bio.FDR.the)/length(trait_gene$bio.FDR.the)
                colnames(trait_gene)[colnames(trait_gene)=='p.est'] <- "Pvalue"         
            }
#load drug database
...
#save results of DEA
    save(DEA,file=paste0('/exeh_4/jinghong_qiu/SO_Lab/DEA/p-version.',x,y,z,'.enrich.rdata'))
    fwrite(as.data.frame(DEA),file=paste0('/exeh_4/jinghong_qiu/SO_Lab/DEA/p-version.',x,y,z,'.enrich.csv'),sep = ',')
...
#target='hippocampal_vol'
    result_file=paste0('/exeh_4/jinghong_qiu/SO_Lab/DEA/p-version.',x,y,z,'.enrich.csv')
...
#write.csv(ATC_enrich_res,"ATC_all_enrichment.csv", quote=F, row.names=F)
    fwrite(as.data.frame(ATC_enrich_res),file=paste0('p-version.',x,y,z,'.category-two.csv'),sep=',')
