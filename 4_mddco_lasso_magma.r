...
# <simpleError in fn(par, ...): Mat::operator(): index out of bounds>
...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_2_lasso/mddco/Result')

#----Loading data----#

#Target
	target.gene=fread('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_1_all/mddco/Result/mddco.genes.out')
...
#Variables
	...
	files=lapply(paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_2_lasso/mddco/Result/',listnames,'.genes.out'),fread,stringsAsFactor=F)
#data preproccess
...
#generate geneID and output for biological annotation#
	...
	fwrite(as.data.frame(gene.list),'mddco.gene.list.txt')
...
#Save result	
	fwrite(as.data.frame(target.gene.order),'mddco.gene.bio.fdrreg.txt',sep=' ')
	fwrite(as.data.frame(assessment_sum),'/exeh_4/jinghong_qiu/SO_Lab/Contribution/magma-mddco.add.csv',sep=',')
	sum_data = data.frame(qval,fdr.the,fdr.emp,bio.fdr.the,bio.fdr.emp,bio.fdr.the.lasso,bio.fdr.emp.lasso)
	fwrite(as.data.frame(sum_data),'/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/magma_mddco.add.csv',sep=',')	
	save(fdr.target.theore,fdr.target.empiri,fdr.target.theore.bio,fdr.target.empiri.bio,fdr.target.theore.bio.lasso,fdr.target.empiri.bio.lasso,file = "/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/magma_mddco.add.RData")
#exit the software
	q('no') 