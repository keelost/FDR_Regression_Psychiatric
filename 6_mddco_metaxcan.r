...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/metaxcan_1_all/mddco')
...
#extract target zscore and construct variables z.score matrix 
	...
	colnames(z.combine)=c('bd2018','cannabis','dx.dep','ed2019','edu','inte','neuroti2018','ocd2018','scz.eas2019','wal.scz2018')
...
#Save final result
	...
	fwrite(as.data.frame(sum_data),paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/meta_mddco_',brain_region,'.test_SetSeed.csv'),sep=',')
	save(fdr.theore,fdr.empiri,fdr.target.theore,fdr.target.empiri,lasso.fdr.target.theore,lasso.fdr.target.empiri, file =paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/meta_mddco_',brain_region,'.test_SetSeed.RData'))
	fwrite(as.data.frame(target.gene.order),file=paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/smultixcan_1_all/mddco/Result/mddco.finalresult.',brain_region,'.test_SetSeed.txt'),sep=' ')
	assessment.final$region = brain_region
	assessment.final.sum = rbind(assessment.final.sum,assessment.final)
	fwrite(as.data.frame(assessment.final),file=paste0('/exeh_4/jinghong_qiu/SO_Lab/Contribution/meta-mddco-',brain_region,'.test_SetSeed.csv'),sep=',')
}
	fwrite(as.data.frame(final_sum),file='/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/meta_mddco.all.test_SetSeed100.csv',sep=',')
	fwrite(as.data.frame(assessment.final.sum),file='/exeh_4/jinghong_qiu/SO_Lab/Contribution/meta-mddco-all.test_SetSeed100.csv',sep=',')
#exit the software
	q('no') 