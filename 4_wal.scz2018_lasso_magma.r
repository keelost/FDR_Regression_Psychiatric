...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_2_lasso/wal.scz2018/Result')

#----Loading data----#

#Target
	target.gene=fread('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_1_all/wal.scz2018/Result/wal.scz2018.genes.out')
	target.gene.order=target.gene[order(target.gene$GENE),]
#Variables
	listnames = c('ad','adhd2019','alco2018','anx','asd2019','bd2018','cannabis','chrono','dx.dep','dx.life.threat.ill','ed2019','ever.csh','ever.sh','ever.st','life.threat.accident','mdd2019','mddco','neuroti2018','no.dep','ocd2018','pd','physical.crime','psycho.experi','ptsd2019','reac','sleep.dura','smoking','st','st.dep','tense','wit.death')
	files=lapply(paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_2_lasso/wal.scz2018/Result/',listnames,'.genes.out'),fread,stringsAsFactor=F)
#data preproccess
...
#generate geneID and output for biological annotation#
	...
	fwrite(as.data.frame(gene.list),'wal.scz2018.gene.list.txt')
...
#Save result	
	fwrite(as.data.frame(target.gene.order),'wal.scz2018.gene.bio.fdrreg.txt',sep=' ')
	fwrite(as.data.frame(assessment_sum),'/exeh_4/jinghong_qiu/SO_Lab/Contribution/magma-wal.scz2018.lasso.csv',sep=',')
	sum_data = data.frame(qval,fdr.the,fdr.emp,bio.fdr.the,bio.fdr.emp,bio.fdr.the.lasso,bio.fdr.emp.lasso)
	fwrite(as.data.frame(sum_data),'/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/magma_wal.scz2018.lasso.csv',sep=',')	
	save(fdr.target.theore,fdr.target.empiri,fdr.target.theore.bio,fdr.target.empiri.bio,fdr.target.theore.bio.lasso,fdr.target.empiri.bio.lasso,file = "/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/magma_wal.scz2018.lasso.RData")
#exit the software
	q('no') 