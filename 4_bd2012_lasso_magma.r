...
#----load packages and set dir----#
	...
	setwd('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_2_lasso/bd2012/Result')

#----Loading data----#

#Target
	target.gene=fread('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_1_all/bd2012/Result/bd2012.genes.out')
	target.gene.order=target.gene[order(target.gene$GENE),]
#Variables
	listnames = c('ad','alco2018','asd2019','cannabis','college2013','dx.dep','ed2019','edu','ever.csh','ever.sh','ever.st','life.threat.accident','mdd2019','mddco','neuroti2018','no.dep','ocd2018','ptsd2019','sleep.dura','wal.scz2018')
	files=lapply(paste0('/exeh_4/jinghong_qiu/SO_Lab/level2/magma_2_lasso/bd2012/Result/',listnames,'.genes.out'),fread,stringsAsFactor=F)
#data preproccess
...
#generate geneID and output for biological annotation#
	...
	fwrite(as.data.frame(gene.list),'bd2012.gene.list.txt')
...
#Save result	
	fwrite(as.data.frame(target.gene.order),'bd2012.gene.bio.fdrreg.txt',sep=' ')
	fwrite(as.data.frame(assessment_sum),'/exeh_4/jinghong_qiu/SO_Lab/Contribution/magma-bd2012.lasso.csv',sep=',')
	sum_data = data.frame(qval,fdr.the,fdr.emp,bio.fdr.the,bio.fdr.emp,bio.fdr.the.lasso,bio.fdr.emp.lasso)
	fwrite(as.data.frame(sum_data),'/exeh_4/jinghong_qiu/SO_Lab/level2/Result_table/magma_bd2012.lasso.csv',sep=',')	
	save(fdr.target.theore,fdr.target.empiri,fdr.target.theore.bio,fdr.target.empiri.bio,fdr.target.theore.bio.lasso,fdr.target.empiri.bio.lasso,file = "/exeh_4/jinghong_qiu/SO_Lab/level2/FDR_models/magma_bd2012.lasso.RData")
#exit the software
	q('no') 